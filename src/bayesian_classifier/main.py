...
value of dataset
"""
def extract_dataset(path, order, skip_first_line=False):
  f = open(path, 'r')
  raw = f.read().split('\n')
  if skip_first_line:
    raw.pop(0)
...
def main():
  ...
  training_data =  extract_dataset(training_file_name, order, skip_first_line=True)
  feat_coll = generate_feature_collection(training_data, config)

  feat_coll.train()
  test_data =  extract_dataset(test_file_name, order, skip_first_line=True)
